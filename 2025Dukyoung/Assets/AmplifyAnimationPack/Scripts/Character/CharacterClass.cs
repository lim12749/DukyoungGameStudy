...
namespace AmplifyAnimationPack
{
	...
	public class CharacterClass : MonoBehaviour
	{
		...
		private void UpdateCamera()
		{
			...
			followCamOffset.Offset = Vector3.Lerp( followCamOffset.Offset , cameraTargetOffset , 0.1f );
#else
			DebugCinemachineError();
#endif
	...
	#region LockOnMethods
		...
		public void SetCameraShakeAmplitude( float _valueToSet )
		{
			...
			{
				cameraNoises[ i ].AmplitudeGain = _valueToSet;
			}
#else
			DebugCinemachineError();
#endif
		...
		public void SetCameraShakeFrequency( float _valueToSet )
		{
			...
			for( int i = 0 ; i < cameraNoises.Length ; i++ )
			{
				cameraNoises[ i ].FrequencyGain = _valueToSet;
			}
#else
			DebugCinemachineError();
#endif
